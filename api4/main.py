...
@app.route("/")
def index():
    return "Usage: http://<hostname>[:<prt>]/apia/<url>"
...
@app.route('/apia/valor', methods=['POST'])
def recibir_datos():
    ...
    if request.method == 'POST':
        ...
        consolidated_dfmi = pd.concat([consolidated_dfmi.query(f'userId == {numerox}'), consolidated_dfmi.iloc[3000:4001]])
...
@app.route('/apia/valor', methods=['GET'])
def get_users():
...
@app.route('/apia/peliculas', methods=['GET'])
def get_peliculas():
...
@app.route('/apia/csv', methods=['GET'])
def get_csv():
    csv_cached = redis_conn.get('csv') 
    if csv_cached:
